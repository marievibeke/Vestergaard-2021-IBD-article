...
# Is alpha diversity important for the following disease score?

Initiate with testing alpha diversity!
```{r echo=T, message=F, error=F, warning=F}
#Read in alpha diversity and merge with metadata
alpha_div <-  read_delim(file=paste0(path, "humann3_processed_output/processed_tables/", "AlphaDiv.txt"), delim="\t")
alpha_div <- merge(alpha_div, meta, by.x="J_ID", by.y="J_ID")
```

```{r}
#Logistic model: disease score (t) ~ disease score (t-1) + alpha div (t-1) + cov (t)

alpha_div$Shannon2 <- (alpha_div$Shannon)^2 
alpha_div$Simpson2 <- sqrt(alpha_div$InvSimpson)

#With clinical disease score
#Select optimizer, that reaches convergence
model1 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Chao1+ (1|person_ID), data=alpha_div, family=binomial)
model2 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon+ (1|person_ID), data=alpha_div, family=binomial)
model3 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+InvSimpson+ (1|person_ID), data=alpha_div, family=binomial)
model4 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon2+ (1|person_ID), data=alpha_div, family=binomial)
model5 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Simpson2+ (1|person_ID), data=alpha_div, family=binomial)

#The models did not converge! Investigate if a different optimizer can solve the problem
#Fit with new optimizer - some still have the isSingular? problem..:
model1 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Chao1+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
model2 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="nlminbwrap"))
model3 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+InvSimpson+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
model4 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon2+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="nlminbwrap"))
model5 <- glmer(score_binary_future~score_binary+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Simpson2+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="nlminbwrap"))


summary(model1)$coefficients
summary(model2)$coefficients #isSingluar?
summary(model3)$coefficients
summary(model4)$coefficients #isSingluar?
summary(model5)$coefficients #isSingluar?
AIC(model1)
AIC(model2)
AIC(model3)
AIC(model4)
AIC(model5)
#l_asa and score before are the best predicters! -> significant

#With paraclinical disease score
#Select optimizer, that eaches convergence
model1 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Chao1+ (1|person_ID), data=alpha_div, family=binomial)
model2 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon+ (1|person_ID), data=alpha_div, family=binomial)
model3 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+InvSimpson+ (1|person_ID), data=alpha_div, family=binomial)
model4 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon2+ (1|person_ID), data=alpha_div, family=binomial)
model5 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Simpson2+ (1|person_ID), data=alpha_div, family=binomial)

#The models 2-5 did not converge! Investigate if a different optimizer can solve the problem:
model2 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
model3 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+InvSimpson+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
model4 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Shannon2+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
model5 <- glmer(binary_fcal_future~binary_fcal+BMI+sex+asa_future+aza_future+l_asa_future+pred_future+Simpson2+ (1|person_ID), data=alpha_div, family=binomial, control=glmerControl(optimizer="bobyqa"))
...
#BMI and score are best predicters -> significant
```
Summary: Alpha diversity does not influence disease score at the following time point!
